...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->db_login = $this->load->database($this->esquema['sinfonica'], TRUE);
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->db_login->select('password, salt')
		                  ->where('id', $id)
		                  ->limit(1)
		                  ->order_by('id', 'desc')
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			$query = $this->db_login->select($this->identity_column)
			...
			$this->trigger_events('extra_where');
			$this->db_login->update($this->tables['users'], $data, array('id' => $id));
		}
		else
		{
			$data = array(
		...
		$return = $this->db_login->affected_rows() == 1;
		if ($return)
		{
	...
	public function clear_forgotten_password_code($code) {
		...
		$this->db_login->where('forgotten_password_code', $code);

		if ($this->db_login->count_all_results($this->tables['users']) > 0)
		{
			$data = array(
			...
			$this->db_login->update($this->tables['users'], $data, array('forgotten_password_code' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db_login->select('id, password, salt')
		...
		$this->trigger_events('extra_where');
		$this->db_login->update($this->tables['users'], $data, array($this->identity_column => $identity));
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->db_login->update($this->tables['users'], $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->db_login->where('username', $username)
										->group_by("id")
										->order_by("id", "ASC")
										->limit(1)
	...
	public function email_check($email = '')
	{
		...
		return $this->db_login->where('email', $email)
										->group_by("id")
										->order_by("id", "ASC")
										->limit(1)
		                ->count_all_results($this->tables['users']) > 0;
	...
	public function identity_check($identity = '')
	{
		...
		return $this->db_login->where($this->identity_column, $identity)
		                ->count_all_results($this->tables['users']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->db_login->update($this->tables['users'], $update, array($this->identity_column => $identity));
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query = $this->db_login->get_where($this->tables['groups'],array('name' => $this->config->item('default_group', 'ion_auth')),1)->row();
		...
		$this->db_login->insert($this->tables['users'], $user_data);
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password,$token, $remember=FALSE)
	{
		...
		$query = $this->db_login->select($this->identity_column . ', email, id, password, active, last_login,co_estatus')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
//                die($this->db_login->last_query());
...
//                        var_dump($user->co_estatus);die();
			...
			if ($password === TRUE)
			{
				...
				->where('u.'.$this->identity_column, $identity)
				->where('a.token', $token)
				->join($this->tables['groups'] . '  g', 'g.id = ug.group_id','left')
				->join($this->tables['app'] . ' a' ,'a.id = ug.app_id','left')
				->join($this->tables['users'] .' u ','u.id = ug.user_id','left')
				->limit(1)
				->from($this->tables['users_groups'] . ' ug');
...
//				die($this->db_login->last_query());
				...
				if ($query->num_rows() === 0)
				{
					$this->set_error('login_unsuccessful_token');
					return FALSE;
				}

				$this->set_session($user,$app_id);
	...
	function get_attempts_num($identity)
	{
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $ip_address = $this->_prepare_ip($this->input->ip_address());
            $this->db_login->select('1', FALSE);
            if ($this->config->item('track_login_ip_address', 'ion_auth')) {
            	$this->db_login->where('ip_address', $ip_address);
            	$this->db_login->where('login', $identity);
            } else if (strlen($identity) > 0) $this->db_login->or_where('login', $identity);
            $qres = $this->db_login->get($this->tables['login_attempts']);
	...
	public function get_last_attempt_time($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->db_login->select_max('time');
            if ($this->config->item('track_login_ip_address', 'ion_auth')) $this->db_login->where('ip_address', $ip_address);
			else if (strlen($identity) > 0) $this->db_login->or_where('login', $identity);
			$qres = $this->db_login->get($this->tables['login_attempts'], 1);
	...
	public function increase_login_attempts($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			$ip_address = $this->_prepare_ip($this->input->ip_address());
			return $this->db_login->insert($this->tables['login_attempts'], array('ip_address' => $ip_address, 'login' => $identity, 'time' => time()));
		}
		return FALSE;
	}
	...
	public function clear_login_attempts($identity, $expire_period = 86400) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->db_login->where(array('ip_address' => $ip_address, 'login' => $identity));
			// Purge obsolete login attempts
			$this->db_login->or_where('time <', time() - $expire_period, FALSE);

			return $this->db_login->delete($this->tables['login_attempts']);
		}
		return FALSE;
	}
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			{
				$this->db_login->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.id', 'inner');
				$this->db_login->where_in($this->tables['groups'] . '.name', $group_names);
			}
			if(!empty($group_ids))
			{
				$this->db_login->{$or_where_in}($this->tables['users_groups'].'.'.$this->join['groups'], $group_ids);
			}
		}
		...
		{
			$this->db_login->limit($this->_ion_limit, $this->_ion_offset);
		...
		{
			$this->db_login->limit($this->_ion_limit);
		...
		{
			$this->db_login->order_by($this->_ion_order_by, $this->_ion_order);

			$this->_ion_order    = NULL;
			$this->_ion_order_by = NULL;
		}

		$this->response = $this->db_login->get($this->tables['users']);
	...
	{
		//die($this->ion_auth->get_app_id());
		$this->trigger_events('get_users_group');

		// if no id was passed use the current users id
		$id || $id = $this->session->userdata('user_id');
		$query=$this->db_login->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
		                ->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
		                ->where($this->tables['users_groups'].'.'.$this->join['app'], $this->ion_auth->get_app_id())
...
//		    die($this->db_login->last_query());           
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		{
			if ($this->db_login->insert($this->tables['users_groups'], array( $this->join['groups'] => (float)$group_id, $this->join['users'] => (float)$user_id)))
			{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			foreach($group_ids as $group_id)
			{
				...
				$this->db_login->delete($this->tables['users_groups'], array($this->join['groups'] => (float)$group_id, $this->join['users'] => (float)$user_id));
				//die($this->db_login->last_query());
		...
		{
			if ($return = $this->db_login->delete($this->tables['users_groups'], array($this->join['users'] => (float)$user_id))) {
	...
	public function groups()
	{
		...
		$this->where($this->tables['groups'].'.'.$this->join['app'], $this->ion_auth->get_app_id());
		...
		$this->response = $this->db_login->get($this->tables['groups']);
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$this->db_login->update($this->tables['users'], $data, array('id' => $user->id));

		if ($this->db_login->trans_status() === FALSE)
		{
			$this->db_login->trans_rollback();
	...
	**/
	...
	public function update_last_login($id)
	{
		...
		$this->db_login->update($this->tables['users'], array('last_login' => time()), array('id' => $id));
	 ...
	 * @author jrmadsen67
	 **/
	public function set_session($user,$app)
	{
	...
	public function remember_user($id)
	{
		...
		$this->db_login->update($this->tables['users'], array('remember_code' => $salt), array('id' => $id));

		if ($this->db_login->affected_rows() > -1)
		{
			// if the user_expire is set to zero we'll set the expiration two years from now.
			if($this->config->item('user_expire', 'ion_auth') === 0)
			{
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db_login->select($this->identity_column.', id, email, last_login')
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->db_login->get_where($this->tables['groups'], array('name' => $group_name))->num_rows();
		if($existing_group !== 0)
		{
		...
		$data = array('name'=>$group_name,'description'=>$group_description,'app_id'=>$this->ion_auth->get_app_id());
		...
		$this->db_login->insert($this->tables['groups'], $data);
		$group_id = $this->db_login->insert_id();
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->db_login->get_where($this->tables['groups'], array('name' => $group_name))->row();
        ...
        $group = $this->db_login->get_where($this->tables['groups'], array('id' => $group_id))->row();
		...
		$this->db_login->update($this->tables['groups'], $data, array('id' => $group_id));
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db_login->delete($this->tables['users_groups'], array($this->join['groups'] => $group_id));
		// remove the group itself
		$this->db_login->delete($this->tables['groups'], array('id' => $group_id));
	...
	{
		$filtered_data = array();
		$columns = $this->db_login->list_fields($table);
	...
	public function existe_email_user($email)
	{
		$query = $this->db_login->select('id')
			                  ->where('email', $email)
			                  ->limit(1)
		    				  ->order_by('id', 'desc')
			                  ->get($this->tables['users']);
	...
	/*
	 * Agregado por Joan Delgado
	 */
	
	public function get_groups_app($app_id=false)
	{
		$app_id || $app_id = $this->session->userdata('app_id');
		$query = $this->db_login->select('id')
		->where($this->join['app'], $app_id)
		->order_by('id', 'desc')
